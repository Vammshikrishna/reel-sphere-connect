...
const DiscussionRooms = () => {
  ...
  }, [selectedCategory, sortBy, defaultCategoryId, toast]);
  ...
  useEffect(() => {
    const initializeCategories = async () => {
      try {
        ...
        if (!generalCat) {
          console.log("'General Discussion' category not found. Creating it...");
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      const newRoomForUI: DiscussionRoom = {
        id: newRoomId,
        title: title,
        ...
        members: [{ profiles: { id: user.id, full_name: user.user_metadata.full_name, avatar_url: user.user_metadata.avatar_url } }]
      };
  ...
  const RoomCard = ({ room, isFeatured = false }: { room: DiscussionRoom, isFeatured?: boolean }) => {
    return (
          ...
          <div className="flex items-center mb-4">
            <div className="flex -space-x-2 overflow-hidden">
              {room.members?.slice(0, 5).map((member, index) => (
                <Avatar key={member?.profiles?.id ?? index} className="inline-block h-6 w-6 rounded-full ring-2 ring-gray-800">
                  <AvatarImage src={member?.profiles?.avatar_url} /><AvatarFallback>{member?.profiles?.full_name?.[0] ?? '?'}</AvatarFallback>
                </Avatar>
              ))}
            </div>
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 sm:p-6 lg:p-8">
      <div className="container mx-auto">
        ...
        <Dialog open={chatOpen} onOpenChange={setChatOpen}>
          <DialogContent className="max-w-4xl h-[80vh] flex flex-col p-0 bg-gray-800 border-gray-700 text-white"><DialogHeader className="p-4 border-b border-gray-700"><DialogTitle className="flex items-center gap-2"><MessageCircle className="h-5 w-5 text-indigo-400" />{rooms.find(r => r.id === selectedRoom)?.title || 'Loading Room...'}</DialogTitle></DialogHeader><div className="flex-1 overflow-y-auto p-0">{selectedRoom && <EnhancedRoomChat roomId={selectedRoom} roomTitle={rooms.find(r => r.id === selectedRoom)?.title || ''} />}</div></DialogContent>
        </Dialog>
        ...
        <Dialog open={createModalOpen} onOpenChange={setCreateModalOpen}>
            <DialogContent className="bg-gray-800 border-gray-700 text-white">
              <DialogHeader><DialogTitle>Create a New Discussion Room</DialogTitle></DialogHeader>
              <div className="space-y-4">
                ...
                <div><Label htmlFor="room-cat">Category</Label><Select value={newRoom.category_id} onValueChange={val => setNewRoom({...newRoom, category_id: val})}><SelectTrigger className="bg-gray-700 border-gray-600"><SelectValue placeholder="Select a category" /></SelectTrigger><SelectContent className="bg-gray-800 border-gray-700 text-white">{categories.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>
