...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { name, description, room_type = "public" } = body as {
      name?: string;
      description?: string;
    ...
    if (insertRoom.error || !insertRoom.data) {
      return new Response(JSON.stringify({ error: insertRoom.error?.message ?? "Failed to create room" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const memberInsert = await supabase.from("room_members").insert({
      room_id: roomData.id,
      user_id: user.id,
    });
    ...
    if (memberInsert.error) {
      ...
      // Attempt cleanup
      const cleanup = await supabase.from("discussion_rooms").delete().eq("id", roomData.id);
      if (cleanup.error) {
        console.error("Failed to cleanup room after member insert failure:", cleanup.error.message);
      }
